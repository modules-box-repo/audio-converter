...
echo -e "${BLUE}[4/4] Checking ffmpeg...${NC}"
if command -v ffmpeg &> /dev/null; then
    echo -e "${GREEN}[4/4] ffmpeg is already installed${NC}"
else
    echo -e "${YELLOW}[4/4] ffmpeg not found${NC}"
    echo -e "${BLUE}[4/4] Installing ffmpeg...${NC}"
    apk add ffmpeg && echo -e "${GREEN}[4/4] ffmpeg installed successfully${NC}"
fi
